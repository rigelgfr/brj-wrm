// api/operation_out/update.ts
import { prisma } from '@/src/lib/prisma';

export async function updateOutboundAggregates() {
  try {
    await prisma.$transaction(async (tx) => {
      await tx.$executeRaw`
        ...
        -- Get all existing combinations from outbound_aggregated
        ...
        INSERT INTO outbound_aggregated (
            warehouse, 
            year, 
            month, 
            week_in_month, 
